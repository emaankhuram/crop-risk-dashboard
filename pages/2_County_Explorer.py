...
st.markdown("### Explore historical trends and risk factors for specific counties")
...
# Climate trends
st.markdown("### 🌡️ Climate Change Indicators")
...
with st.expander("💡 What Do These Metrics Mean?"):
    st.markdown(f"""
    ### Interpreting {county_name}, {state_name}
    
    **Volatility Change: {cv_change:.2f}%**
    - This county's yield variability {'increased' if cv_change > 0 else 'decreased'} by {abs(cv_change):.2f} percentage points
    - {'⚠️ Higher volatility = less predictable yields' if cv_change > 0 else '✅ Lower volatility = more stable production'}
    
    **Temperature Variability: {temp_std_change:.2f}°C**
    - Temperature swings during growing season {'increased' if temp_std_change > 0 else 'decreased'} by {abs(temp_std_change):.2f}°C
    - Research shows erratic temperatures stress crops more than gradual warming
    
    **NDVI Changes: {ndvi_change:.3f}**
    - Vegetation health index {'declined' if ndvi_change < 0 else 'improved'}
    - Lower NDVI often indicates drought stress or crop health issues
    
    ### Adaptation Strategies
    {'Recommendations for high-risk counties like this one:' if cv_change > 5 else 'This county shows resilience. Key practices to maintain:'}
...
if cv_change > state_avg:
    st.warning(f"⚠️ This county is experiencing **{cv_change - state_avg:.2f}% more** volatility increase than the state average")
else:
